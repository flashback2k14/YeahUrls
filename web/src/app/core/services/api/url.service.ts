...
export class UrlService {
  ...
  getUrlsByUser (userId: string): Promise<Array<Url>> {
    return new Promise((resolve, reject) => {
      this._http.get<Array<Url>>(`${this._baseUrl}/${userId}`, {
  ...
  putUrlByUserAndId (userId: string, urlId: string, urlData: any): Promise<Url> {
    return new Promise((resolve, reject) => {
      this._http.put<Url>(`${this._baseUrl}/${userId}/${urlId}`, urlData, {
        headers: this._headers.append("X-Access-Token", localStorage.getItem(StorageKeys.USERTOKEN))
      })
        .subscribe(result => resolve(result),
        error => reject(error));
  ...
  postUrlByUser (userId: string, urlData: any): Promise<Url> {
    return new Promise((resolve, reject) => {
      this._http.post<Url>(`${this._baseUrl}/${userId}`, urlData, {
        headers: this._headers.append("X-Access-Token", localStorage.getItem(StorageKeys.USERTOKEN))
      })
        .subscribe(result => resolve(result),
        error => reject(error));
  ...
  deleteUrlByUserAndId (userId: string, urlId: string): Promise<string> {
    return new Promise((resolve, reject) => {
      this._http.delete<string>(`${this._baseUrl}/${userId}/${urlId}`, {
        headers: this._headers.append("X-Access-Token", localStorage.getItem(StorageKeys.USERTOKEN))
      })
        .subscribe(result => resolve(result),
        error => reject(error));
