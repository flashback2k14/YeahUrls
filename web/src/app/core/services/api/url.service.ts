...
export class UrlService {
  ...
  constructor(_configService: ConfigService, private _http: HttpClient) {
    ...
    this._headers = new HttpHeaders().append("accept", "application/json").append("content-type", "application/json");
  }
  ...
  getDuplicateUrls(userId: string): Promise<Array<DuplicateUrlLean>> {
    return new Promise((resolve, reject) => {
      this._http
        .get<Array<DuplicateUrlLean>>(`${this._baseUrl}/duplicates/${userId}?lean=true`, {
          headers: this._headers.append("X-Access-Token", localStorage.getItem(StorageKeys.USERTOKEN))
        })
        .subscribe(result => resolve(result), error => reject(error));
    });
